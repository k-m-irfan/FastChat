...
def speech_pipeline(filepath,source,target,temperature,top_p,max_new_tokens,model_name):
    try:
        user_prompt = seamless.run_s2tt(input_audio=filepath, source_language = source, target_language = source)
        user_prompt_translated = seamless.run_t2tt(input_text=user_prompt, source_language = source, target_language = "English")

        # assistant_response = "This is a test response"
        assistant_response = bot_response_speech(user_prompt_translated,model_name,temperature,top_p,max_new_tokens, gr.Request)
        # assistant_response = generate_response(user_prompt_translated)

        response_audio, assistant_response_translated = seamless.run_t2st(input_text = assistant_response, source_language = "English", target_language = target)
    except:
        user_prompt, response_audio, assistant_response_translated = None, None, None
    return user_prompt, assistant_response_translated, response_audio
...
def model_worker_stream_iter_speech(
    model_name,
    worker_addr,
    prompt,
    temperature,
...
def generate_response(input_message,model_name):
    model_name = "lmsys/vicuna-7b-v1.5"


    controller_addr = "http://localhost:21001" # default
    ...
    conv.append_message(conv.roles[0], input_message)
...
def bot_response_speech(
    ...
    # if apply_rate_limit:
    #     ret = is_limit_reached(model_name, ip)
    #     if ret is not None and ret["is_limit_reached"]:
    #         error_msg = RATE_LIMIT_MSG + "\n\n" + ret["reason"]
    #         logger.info(f"rate limit reached. ip: {ip}. error_msg: {ret['reason']}")
    #         return
    ...
    if model_api_dict is None:
        ...
        stream_iter = model_worker_stream_iter_speech(
            model_name,
            worker_addr,
            prompt,
            temperature,
    ...
    with gr.Tabs():
        ...
        with gr.Tab(label="Speak"):
                ...
                with gr.Column():
                    with gr.Group():
                        with gr.Row():
                            ...
                            #     choices=seamless.COMMON_LANGUAGE_NAMES,
                            #     value="English",
                            # )
            ...
            submit_button.click(
                ...
                inputs=[input_audio,language,language,temperature,top_p,max_output_tokens,model_selector],
                outputs=[user_prompt,assistant_response,output_audio]
            )
